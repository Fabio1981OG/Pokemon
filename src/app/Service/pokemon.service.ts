...
export class PokemonService {
      ...
      map(pokemons => {
        return pokemons.map(pokemon => {
          return {
            ...
            imgURL: `${this.imgUrl}${pokemon.id}.png` 
          };
        });
      })
    );
  ...
  // getProcurarPalavra(palavra: string): Observable<Pokemon[]> {
  //   return this.http.get<Pokemon[]>(
  //     `${this.pokemonUrl}/contains/${palavra}`,
  //     this.httpOptions
  //   ).pipe(
  //     tap(_ => this.log(`procurou por pokemons contendo "${palavra}"`)),
  //     catchError(this.handleError<Pokemon[]>('getProcurarPalavra', []))
  //   );
  // }
  }